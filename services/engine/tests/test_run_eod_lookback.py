...
def test_builtin_and_model_paths_bound_to_max_lookback_plus_buffer():
    # 内置因子 max 回看 = mom20(20),+5 缓冲 = 25;模型路径用同一批内置因子,同界。
    assert run_eod_lookback(None, None) == 25
    assert run_eod_lookback({"coef": {"f_bp": 1.0}}, None) == 25
...
def test_invalid_custom_expr_falls_back_to_builtin_bound():
    ...
    assert run_eod_lookback(None, [{"name": "evil", "expr": "__import__('os')"}]) == 25
