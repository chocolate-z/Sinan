...
test('CORS 放行 Tauri webview origin(含 Windows http://tauri.localhost),否则打包版前端连不上 api', async () => {
  const { app, fastify } = await build();
  try {
    // Windows(WebView2)= http://tauri.localhost;macOS/Linux = tauri://localhost;dev = http://localhost:9521。
    for (const origin of ['http://tauri.localhost', 'tauri://localhost', 'http://localhost:9521']) {
      const r = await fastify.inject({
        method: 'OPTIONS',
        url: '/api/v1/providers/active',
        headers: {
      ...
      const allowOrigin = String(r.headers['access-control-allow-origin'] ?? '');
      assert.equal(
        allowOrigin,
        origin,
        `CORS 应放行 origin ${origin},实际 allow-origin:"${allowOrigin}"`,
      );
    }
  } finally {
    await app.close();
