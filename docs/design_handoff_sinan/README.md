...
**2. 行情 Market** (`market`)
- 左(352px):可搜索报价表(代码/名称/现价/涨跌幅,PnL 色,行可选中高亮)。
- 右:个股大盘头(名称+代码+现价+涨跌)、6 宫格关键指标(今开/最高/最低/量/额/PE)、MA 图例、**K线蜡烛图 + MA5/10/20 均线 + 成交量子图**、分时/日/周/月 K 与前复权/后复权/不复权切换。
- 蜡烛:涨=PnL红 跌=PnL绿(随反转开关)。
