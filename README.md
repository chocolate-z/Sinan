...
## ⚠️ 先读:这套方法能不能稳定盈利?(alpha-lab 实测结论)

司南的方法论(基本面 + 多因子打分 + 风控纪律)来自配套研究项目 **alpha-lab**。该项目用最严的诚实尺子——**足额成本 + 防未来函数 + 样本内外 + Walk-Forward + deflated Sharpe(多重检验)+ 风格归因**——把"经典多因子长多头"从头到尾验了一遍。结论必须先说清:

> **以个人投资者的数据与体量,这套方法不足以做出"靠 skill 战胜市场"的稳定盈利策略。这些数据并不支持稳定盈利。**

实测要点(完整证据见 `alpha-lab/docs/研究结论与进展.md`):

- **横截面 IC 是真的**:低换手 / 低波 / 反转 / 价值等因子确有预测力,因子质检功能有效。
- **但做成"长多头跑赢市值加权大盘",拿不出统计稳健的超额**:实时 walk-forward 超额 **−0.75%/年**,deflated Sharpe **不显著(7%)**;之前看着像 alpha 的 +3.96%/年,拆开是**选择偏差 + 多重检验放大的小盘 size beta**。
- **剔掉已知风格后,残余 alpha 显著为负**(−6.67%/年,t=−2.70)——**没有独有 alpha,赚的是风格 beta**(尤其小盘,容量 / 壳 / 监管受限)。
- 进一步逐一证伪(六颗"边界暗礁"):多空对冲(真,但 80% 的边在**不可融券**的小盘空头)、北向(拥挤,被实时套利)、分析师 revision(数据接口 **2 次/天** 拉不动)、短线时序反转(真,但 **1–2 天即衰减**、和 HFT 争食)。**可实现的真 alpha,都落在个人够不着 / 拿不稳的角落。**

**所以司南诚实的定位是:**

- ✅ **研究 + 纪律工具**:透明排序、因子质检、诚实样本外回测、**风格归因("看清你在赌什么")**、用纪律对抗追涨杀跌——这些真有价值,且**不依赖 alpha**。
- ✅ 唯一可落地的是**小盘 + 价值 beta**(长期略正期望,但波动大、有 **−30% 级回撤、并不"稳定"**,且一只低费率指数基金能拿到大头)。
- ❌ **不是** alpha 发生器、不是荐股、不是赚钱机器。任何"绝对收益是绿的"都可能只是 beta —— **该看的是超额与 IR**(回测现已自动给一句白话判词:超额 / IR 为负即判"跑输基准,别被绝对收益误导")。

> **一句话:司南最大的价值,是它敢诚实地告诉你"这套方法赚不到独有 alpha、你赚的是 beta",从而帮你不去追假 alpha、不被漂亮回测误导、不在自己身上亏钱。** 在 A 股个人量化这件事上,"诚实地知道边界在哪"本身就是最稀缺的产出——这正是 alpha-lab 与司南的共同结论。
